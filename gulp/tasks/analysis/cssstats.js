...
gulp.task('cwel-analysis-css-stats', () => {
    // Grab the file generated by `cwel-analysis-generate-css-stats-data` task
    const cssstatsJson = JSON.parse(fs.readFileSync(path.join(process.cwd(), 'Cwel', '.tmp', 'docs', 'cssstats', 'cssstats.json'), 'utf-8'));

    // Task 1 - Break build if total size of CSS is over budget
    ...
    // Task 2 - Break build if X percent of rules surpass Y specificity threshold
